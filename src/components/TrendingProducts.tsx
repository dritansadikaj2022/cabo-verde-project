...
export const TrendingProducts: React.FC<TrendingProductsProps> = ({
  ...
  const trending = products.filter((p) => p.isTrending || p.category === 'lighting').slice(0, 4);

  return (
    <section id="trending-section" className="py-16 sm:py-24 bg-neutral-50/70 border-t border-neutral-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          <div className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-red-600 bg-red-50 px-3.5 py-1 rounded-full mb-2 border border-red-100">
            ...
            <span>High Demand Architectural Accents &bull; Large View</span>
          </div>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black text-neutral-900 tracking-tight">
            Trending Products
          </h2>
          <p className="text-neutral-500 text-xs sm:text-sm mt-1">
            Top architectural lighting and accent pieces currently sought after by our community.
          </p>
        </div>

        {/* 4-col Grid with Large Images (NO PRICES, NO ADD TO CART) */}
          ...
          {trending.map((product) => {
            ...
            return (
              ...
              >
                ...
                >
                  {product.badge && (
                    <span className="absolute top-3 left-3 z-10 bg-neutral-900 text-white text-[10px] font-black px-3 py-1 rounded uppercase tracking-wider shadow-xs">
                      {product.badge}
                    </span>
                  )}
